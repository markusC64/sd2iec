/* sd2iec - SD/MMC to Commodore serial bus interface/controller
   ...
   lcd.h: Settings for LCD screen and LCD type autodetect
*/
...
#ifndef F_CPU
#  error "CPU frequency must be defined in the config for delay cycle calculation."
#endif
...
#ifdef LCD_RW_PIN
// All HW variants with RW (except is Arduino SD2IEC LCD)
...
//LCD keypad shield detection routine
        // Port B4 set to input and pullup on
        OPTIONDDR &= ~((1<<OPTION_PORT) );
        OPTIONPORT |= ((1<<OPTION_PORT) );
        lcd_autodetect = 0;//No LCD
if (bit_is_clear(OPTIONPIN,OPTION_PORT)) {
        lcd_autodetect = 4;//HD44780 display
}
